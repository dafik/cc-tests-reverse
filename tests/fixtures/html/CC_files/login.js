        ...
        if ($.validator) {
            $('.login-form').validate({
                ...
                submitHandler: function (form) {
                    ...
                    var url = '/login';
                    var login = $.post(url, {username: username, password: password}, function (output) {
                        if (typeof output == "object") {
                            if (typeof output.reload != "undefined" && output.reload && typeof output.url != "undefined") {
                                window.location.href = output.url;
                            } else {
                                $('.login-form .alert-danger-login i').after(' ' + output.formMessage);
                                $('.login-form .alert-danger-login').show();
                            }
                        } else {
                            $('.login-form .alert-danger-login i').after(' ' + 'unknown error');
                            $('.login-form .alert-danger-login').show();
                        }
                    }).fail(function (jqXHR, textStatus, error) {

                        $('.login-form .alert-danger-login i').after(' ' + jqXHR.responseText);
                        $('.login-form .alert-danger-login').show();
